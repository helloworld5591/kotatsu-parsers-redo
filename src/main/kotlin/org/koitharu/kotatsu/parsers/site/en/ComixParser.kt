     ...
     import okhttp3.Response   // <--- NEW IMPORT                                                                                                                                            
     ...
     internal class Comix(context: MangaLoaderContext) :                                                                                                                                     
         ...
         /                                                                                                                                                                                   
           Intercept all OkHttp requests for this parser.                                                                                                                                    
                                                                                                                                                                                             
           comix.to is behind Cloudflare Bot Management. Android WebView (used for                                                                                                           
           chapters/pages) shares its cookie store with the system browser, so                                                                                                               
           cf_clearance is usually already present after the user has visited the                                                                                                            
           site once in Chrome. OkHttp has its own cookie jar and doesn't see those                                                                                                          
           cookies automatically. This interceptor bridges the gap by:                                                                                                                       
             1. Reading cookies from the WebView cookie store for this domain.                                                                                                               
             2. Merging the cf_clearance (and session) cookies into the OkHttp request.                                                                                                      
             3. Adding the full set of browser-like headers that Cloudflare expects,                                                                                                         
                so the TLS fingerprint mismatch matters less at the HTTP layer.                                                                                                              
                                                                                                                                                                                             
           If no cf_clearance is available (first install, cleared cookies, etc.)                                                                                                            
           the request is forwarded as-is. The WebView-based paths (getPages,                                                                                                                
           getChapters) will still work in that case; the list/detail views will                                                                                                             
           continue to return 403 until the user opens the site in a browser.                                                                                                                
          /                                                                                                                                                                                  
         override fun intercept(chain: Interceptor.Chain): Response {                                                                                                                        
             val original = chain.request()                                                                                                                                                  
                                                                                                                                                                                             
             // Only inject for requests to this domain                                                                                                                                      
             if (!original.url.host.endsWith("comix.to")) {                                                                                                                                  
                 return chain.proceed(original)                                                                                                                                              
             }                                                                                                                                                                               
                                                                                                                                                                                             
             val requestBuilder = original.newBuilder()                                                                                                                                      
                                                                                                                                                                                             
             // --- 1. Inject cf_clearance from the WebView / system cookie store ---                                                                                                        
             val webViewCookies = context.getWebViewCookies("https://comix.to")                                                                                                              
             if (!webViewCookies.isNullOrBlank()) {                                                                                                                                          
                 // Extract only the cookies Cloudflare cares about so we don't                                                                                                              
                 // accidentally send stale session cookies from a different context.                                                                                                        
                 val cfCookies = webViewCookies                                                                                                                                              
                     .split(";")                                                                                                                                                             
                     .map { it.trim() }                                                                                                                                                      
                     .filter { cookie ->                                                                                                                                                     
                         cookie.startsWith("cf_clearance=") ||                                                                                                                               
                             cookie.startsWith("cf_bm=") ||                                                                                                                                  
                             cookie.startsWith("__cf_bm=") ||                                                                                                                                
                             cookie.startsWith("session=")                                                                                                                                   
                     }                                                                                                                                                                       
                     .joinToString("; ")                                                                                                                                                     
                                                                                                                                                                                             
                 if (cfCookies.isNotBlank()) {                                                                                                                                               
                     // Merge with any existing Cookie header set by the OkHttp cookie jar                                                                                                   
                     val existing = original.header("Cookie")                                                                                                                                
                     val merged = if (existing.isNullOrBlank()) cfCookies else "$existing; $cfCookies"                                                                                       
                     requestBuilder.header("Cookie", merged)                                                                                                                                 
                 }                                                                                                                                                                           
             }                                                                                                                                                                               
                                                                                                                                                                                             
             // --- 2. Add browser-like headers Cloudflare passive-checks ---                                                                                                                
             // These must be consistent with the User-Agent to avoid fingerprint                                                                                                            
             // mismatches at the HTTP layer (the TLS layer is handled by the system).                                                                                                       
             requestBuilder                                                                                                                                                                  
             ...
             // --- 3. If still blocked, surface a clear message ---                                                                                                                         
             if (response.code == 403) {                                                                                                                                                     
                 ...
                 if (body.contains("cloudflare", ignoreCase = true) ||                                                                                                                       
                     ...
                     response.close()                                                                                                                                                        
                     requestCloudflareVerification("https://comix.to/")                                                                                                                      
                 }                                                                                                                                                                           
             }                                                                                                                                                                               
